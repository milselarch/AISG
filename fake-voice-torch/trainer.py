...
class Trainer(object):
    def __init__(
        ...
        self.dirpath = '../fake-voice-detection/DS_10283_3336/LA/'
    ...
    def prepare_batch(
        self, batch_size=16, fake_p=0.5, target_lengths=(128, 1024)
    ):
        num_fake = int(batch_size * fake_p)
        num_real = batch_size - num_fake
        fake_filepaths = self.get_rand_filepaths(1, num_fake)
        real_filepaths = self.get_rand_filepaths(0, num_real)
        ...
        target_length = random.choice(range(
            target_lengths[0], target_lengths[1]
        ))
    ...
    def get_rand_filepaths(self, label=0, episodes=10):
        filepaths = []
        for k in range(episodes):
            filepaths.append(self.get_rand_filepath(label))

        return filepaths

    def get_rand_filepath(self, label=0):
        assert label in (0, 1)
