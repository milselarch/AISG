...
try:
    from CustomBatchNorms import BooleanVar, Norm2d
...
def _make_divisible(v, divisor, min_value=None):
    ...
    It ensures that all layers have a channel number that
    is divisible by 8
    It can be seen here:
...
def conv_3x3_bn(inp, oup, stride, is_training):
    return nn.Sequential(
        nn.Conv2d(inp, oup, 3, stride, 1, bias=False),
        Norm2d(oup, is_training=is_training),
        SiLU()
    )


def conv_1x1_bn(inp, oup, is_training):
    return nn.Sequential(
    ...
    def __init__(
        ...
        if use_se:
            self.conv = nn.Sequential(
                ...
                Norm2d(hidden_dim, is_training=is_training),
                SiLU(),
                # dw
                nn.Conv2d(
...
class EffNetV2(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width_mult=1.):
        ...
        layers = [conv_3x3_bn(3, input_channel, 2, self.train_var)]
        # building inverted residual blocks
        block = functools.partial(
            MBConv, is_training=self.train_var
        )
        ...
        output_channel = (
            _make_divisible(1792 * width_mult, 8)
            if width_mult > 1.0 else 1792
        )

        self.conv = conv_1x1_bn(
            input_channel, output_channel,
            is_training=self.train_var
        )
