"""
https://github.com/HHTseng/video-classification/blob/99ebf204f0b1d737e38bc0d8b65aca128a57d7b1/ResNetCRNN_varylength/functions.py#L103
"""
...
class Dataset_CRNN_varlen(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        channels = len(
            self.transform.__dict__['transforms'][2].__dict__['mean'])  # get number of channels from Transformation

        selected_frames = np.intersect1d(np.arange(1, video_len + 1), select) if self.set_frame[
                                                                                     'begin'] < video_len else []

        # Load video frames
        X_padded = torch.zeros((len(select), channels, img_size[0],
                                img_size[1]))  # input size: (frames, channels, image size x, image size y)
